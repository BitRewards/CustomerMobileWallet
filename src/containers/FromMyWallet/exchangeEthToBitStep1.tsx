...
interface DispatchProps {
  openDepositEthToBitStep2: (partnerKey: string, ethereumAddress: string) => void;
}
  ...
  openDepositEthToBitStep2 = () => {
    ...
    if (isValidEthereumAddress(ethereumAddress)) {
      if (typeof this.partnerKey === 'string' && this.partnerKey.length > 0) {
        ...
        Api.postUpdateEthereumAddress(ethereumAddress || '').then(() => {
          openDepositEthToBitStep2(partnerKey, ethereumAddress);
        }).catch((error) => {
  ...
  render() {
    ...
    return (
      <SafeAreaView style={styles.safeContainer}>
        <ScrollView style={styles.safeContainer} keyboardShouldPersistTaps='handled'>
          <View style={styles.container}>
            ...
            <View style={styles.btnContainer}>
              <PrimaryButton onPress={this.openDepositEthToBitStep2} title='Next' />
            </View>
          </View>
...
const mapDispatchToProps = (dispatch: Dispatch) => ({
  openDepositEthToBitStep2: (partnerKey: string, ethereumAddress: string) => dispatch(NavigationActions.openDepositEthToBitStep2(partnerKey, ethereumAddress)),
});
